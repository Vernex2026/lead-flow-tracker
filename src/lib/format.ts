...
export const fmtDate = (iso: string) => format(parseISO(iso), "d LLL yyyy", opts);
export const fmtDateTime = (iso: string) => format(parseISO(iso), "d LLL yyyy, HH:mm", opts);
...
export const dayLabel = (iso: string) => {
  ...
  return format(d, "EEEE, d LLLL yyyy", opts);
};
